...
export class HelpCenterMockApi {
    ...
    private _guideContent: any[] = guideContentData;
    ...
    registerHandlers(): void {
        ...
        this._fuseMockApiService
            ...
            .reply(({ request }) => {
                ...
                result.guides[0]['content'] = this._guideContent.filter((x: any) => x.slug === guideSlug).map((x: any) => x.content);
                result.guides[0]['url'] = this._guideContent.filter((x: any) => x.slug === guideSlug).map((x: any) => x.url);
